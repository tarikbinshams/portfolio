...
export default function EditBlog({ blog }: { blog: any }) {
  ...
  const [readingTime, setReadingTime] = useState<any>(blog?.reading_time);
  ...
  const handleSubmit = async () => {
    ...
    if (image) {
      ...
      formData.append("is_active", blog?.is_active);
    } else {
      formData.append("id", blog?.id);
      formData.append("title", title);
      formData.append("description", description);
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => BlogServiceInstance.updateBlog(data),
    onMutate: (variables) => {
      setLoading(true);
    },
    ...
    <div className="t-w-full t-shadow-[0px_3px_20px_1px_rgba(0,0,0,0.15)] t-rounded-[6px] t-px-6 lg:t-px-20 t-py-4">
      ...
      <div className="t-my-8">
        <div className="t-grid lg:t-flex t-gap-8">
          <div className="t-relative ">
            <input
              id="inputField"
              type="text"
              className="t-w-full lg:t-w-[700px] t-h-[46px] t-border t-border-[#7E7982] t-rounded-lg t-px-2 t-py-1 t-bg-transparent focus:t-outline-none focus:t-ring-1 focus:t-border-[#2B79D3]"
              value={title}
          ...
          <div className="t-relative ">
            <input
              id="inputField"
              type="text"
              className="t-w-full lg:t-w-[380px]  t-h-[46px] t-border t-border-[#7E7982] t-rounded-lg t-px-2 t-py-1 t-bg-transparent focus:t-outline-none focus:t-ring-1 focus:t-border-[#2B79D3]"
              value={tagValue}
            ...
            <div className="t-flex t-absolute t-top-[16px] t-right-2 ">
              {tags.map((tag, index) => (
                ...
                >
                  <div className="t-mx-1.5 t-text-[11px] t-border t-border-[#bdbdbd] t-px-[6px] t-rounded-[4px]">
                    {tag}
                  </div>
                  <svg
        ...
        <div className="t-grid t-grid-cols-1 lg:t-grid-cols-6 lg:t-gap-20 t-mt-8">
          ...
          <div className="t-col-span-4 t-mt-8 lg:t-mt-0">
            <FileUploader file={setImage} currentFile={blog?.image} />
          </div>
        </div>
